...
EventUtil.addHandler(window, "load", () => {
    ...
    EventUtil.addHandler(unshift, "click", () => {
    ...
    EventUtil.addHandler(push, "click", () => {
        let val = text.value.trim();
        if (isNumber(val)) {
    ...
    EventUtil.addHandler(shift, "click", () => {
        if (deque.length) {
            console.log(`shift ${deque.shift()}`);
            dequeDisplay.innerHTML = RenderDeque.render(deque);
        }
    });
    EventUtil.addHandler(pop, "click", () => {
        if (deque.length) {
            console.log(`pop ${deque.pop()}`);
            dequeDisplay.innerHTML = RenderDeque.render(deque);
        }
    });
    EventUtil.addHandler(dequeDisplay, "click", (event) => {
